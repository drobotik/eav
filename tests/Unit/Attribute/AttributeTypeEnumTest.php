...
class AttributeTypeEnumTest extends TestCase
{
    /** @test */
    ...
    public function validation_rule() {
        ...
        $this->assertEquals([
            ...
            "min:1",
            "max:191"
        ], ATTR_TYPE::STRING->validationRule());
        $this->assertEquals([
            "min:1",
            "max:1000"
    ...
    public function get_case() {
        $this->assertEquals(
            ATTR_TYPE::INTEGER,
            ATTR_TYPE::getCase(ATTR_TYPE::INTEGER->value())
        );
        $this->assertEquals(
            ATTR_TYPE::DATETIME,
            ATTR_TYPE::getCase(ATTR_TYPE::DATETIME->value())
        );
        $this->assertEquals(
            ATTR_TYPE::DECIMAL,
            ATTR_TYPE::getCase(ATTR_TYPE::DECIMAL->value())
        );
        $this->assertEquals(
            ATTR_TYPE::STRING,
            ATTR_TYPE::getCase(ATTR_TYPE::STRING->value())
        );
        $this->assertEquals(
            ATTR_TYPE::TEXT,
            ATTR_TYPE::getCase(ATTR_TYPE::TEXT->value())
        );
    }
}
